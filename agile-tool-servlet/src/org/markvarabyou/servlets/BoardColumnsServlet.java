...
/**
 * Servlet class for board columns.
 * User: Mark Varabyou
 * Date: 12/2/13
 * Time: 3:13 PM
 */
public class BoardColumnsServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (id == null)
        {
            String json = null;
            try {
                json = services.getBoardColumnsService().get();
        ...
        try {
            String json = services.getBoardColumnsService().get(id);
            utilities.setJsonToResponseBody(response, json);
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            String outputJson = services.getBoardColumnsService().post(inputJson);
    ...
    public void doPut(HttpServletRequest request, HttpServletResponse response) {
        ...
        {
            try {
                String outputJson = services.getBoardColumnsService().put(id, inputJson);
                utilities.setJsonToResponseBody(response, outputJson);
    ...
    public void doDelete(HttpServletRequest request, HttpServletResponse response) {
        Integer id = utilities.getIdFromRequestParameter(request);
        try {
            services.getBoardColumnsService().delete(id);
        } catch (EntityNotFoundException e) {
            response.setStatus(404);
        } catch (ServiceSetupException e) {
            response.setStatus(400);
